...
export function renderHtml(config, payload, now) {
  const at = now === undefined ? Date.now() : now;
  const separator = config.separator === undefined ? ' | ' : config.separator;
  const joiner = '<span class="sl-sep">' + escapeHtml(separator).replace(/ /g, '&nbsp;') + '</span>';
  return buildLines(config, payload, at)
    .map(parts => parts
      .map(p => {
        const styles = [];
